...
public class EnemyController_sky : MonoBehaviour
{
    //プレイヤーが近づくと追いかける仕組みにする
    bool isActive = false;　//trueになるとプレイヤーを追いかける
    public float speed = 0.5f; //プレイヤーを追いかけるスピード
        ...
        if (player != null)　//プレイヤーが存在する
        {
            ...
            if (isActive)
            {
                ...
                if (axisH > 0.0f)　//横軸がプラスの値なら
                    ...
                    {
                        transform.localScale = new Vector2(-8, 8); //右向き
                    }
                    else
                    {
                    ...
                    if (SceneManager.GetActiveScene().name == "Stage2")
                    {
                        transform.localScale = new Vector2(8, 8);  //左向き
                    }
                    else
                    {
